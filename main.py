...
class RectArray:
    def __init__(self, rt, arr, (x0, y0, wight, height), bord):
        self.array = arr
        self.canvas = Tkinter.Canvas (rt, width=wight + 2 * bord, height=height + 2 * bord, bg='white')
        ...
        self.y = lambda n: (1. * n / self.m * (self.y1 - self.y0) + self.y0) if (n != 0) else self.y0 - 1
        self.rgb = lambda n: (int (255. * n / self.m), int (255. * (self.m - n) / self.m), 0)
        self.rgb_str = lambda n: '#{0[0]:02x}{0[1]:02x}{0[2]:02x}'.format (self.rgb (n))
        self.rect_array = self.create_array ()
        self.out_steps = arr
        self.played = False
        self.sort_str = sys.argv[1] if len(sys.argv) > 2 else "quicksort"
        self.sort = dict(zip(["quicksort","shaker_sort"],[quicksort,shaker_sort]))[self.sort_str]
...
if __name__ == "__main__":
    ...
    border = 50
    rects = RectArray (root, a, (border, h - border, w - 2 * border, h - 2 * border), border)
